        ...
        while self.is_running:
            ...
            if len(unique_colors) != 1:
                ...
                if key_bind is not None:
                    ...
                    self.output_signal.emit({"key_bind": key_bind, "color": str(pixel_color)})
                else:
...
class SettingInterface(ScrollArea):
    ...
    class SelectSpecializationCard(HeaderCardWidget):
        ...
        def __init__(self, parent=None):
            ...
            self.vBoxLayout.setContentsMargins(0, 0, 0, 0)
            if DEV_MODE:
                self.SPECIALIZATION = ["DeathKnightBlood[Hekili]", "PriestDiscipline", "PriestDiscipline[beta]", "DemonHunterHavoc", "DemonHunterHavoc[Hekili]", "DemonHunterVengeance",
                                       # "DeathKnightFrost", "DeathKnightUnholy", "DruidBalance", "DruidFeral", "DruidGuardian", "DruidRestoration", "EvokerAugmentation",
        ...
        def load_config(self):
            ...
            try:
                if DEV_MODE:
                    ...
                    key_file = BASE_DIR.joinpath(f"{selected_specialization}.rkf")
                    if key_file.exists():
                        key_file.unlink()
                    ...
                    obj = file2obj(BASE_DIR.joinpath(f"{selected_specialization}.rkf"))
                ...
                rotation_content_macros = extract_prop_arguments(rotation_content, "Cast")
                rotation_content_macros = sorted(list(set(rotation_content_macros)))
                ...
                interface_path = game_path.joinpath("Interface").joinpath("AddOns").joinpath("Shion")
    ...
    class GamePathCard(HeaderCardWidget):
        def __init__(self, parent=None):
            ...
            self.hBoxLayout.addWidget(self.gamePathSelectButton, 0, Qt.AlignLeft)
            self.vBoxLayout.addLayout(self.hBoxLayout)
    ...
    def activate_software(self):
        ...
        self.vBoxLayout.addWidget(self.selectSpecializationCard, 0, Qt.AlignTop)
...
class SpellSettingInterface(ScrollArea):
    class EditCard(SimpleCardWidget):
        def __init__(self, title, placeholder, code, parent=None):
            ...
            self.saveButton.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Expanding)
    ...
    def import_card(self, card_list):
        for card in card_list:
            card_widget = self.EditCard(card["title"], card["placeholder"], card["code"], self)
...
class RunningInterface(ScrollArea):
    ...
    def handle_output(self, output):
        if "error" in output:
            ...
            self.add_log("按键：" + output["key_bind"] + "  颜色:" + output["color"])
...
class Window(MSFluentWindow):
    ...
    def initNavigation(self):
        ...
        self.addSubInterface(self.spellSettingInterface, FluentIcon.APPLICATION, '技能设置')
