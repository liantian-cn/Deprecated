...
-- 战斗前血线标准
local PreCombatHealScore = 0.90
-- 自我救助标准
local SelfHealScore = 0.5
-- 渐隐术血线标准
local FadeHealScore = 0.85
-- 单体苦修占比
local SinglePenancePercent = 0.75
...
ShionVars["IgnoreTank"] = false;
local function ToggleIgnoreTank()
    ShionVars["IgnoreTank"] = not ShionVars["IgnoreTank"];
    ShionCB["IgnoreTank"]:SetChecked(ShionVars["IgnoreTank"]);
end
CreateLine("忽略tanks", "IgnoreTank", ToggleIgnoreTank)
...
local function calculateUnitHealthScore(unitName, role, classID)
    ...
    local currentUnitIncomingHeal = UnitGetIncomingHeals(unitName, "player")
    ...
    if numGroupMembers > 0 then
        for i = 1, numGroupMembers do
            ...
            if UnitExists(unitName) and not (UnitIsDeadOrGhost(unitName)) and C_Spell.IsSpellInRange(139, unitName) then
                if not (ShionVars["IgnoreTank"] and (UnitGroupRolesAssigned(unitName) == "TANK")) then
...
-- 摘取出玩家状态
local function getPlayerTable(partyHealthScore)
    for _, unitTable in ipairs(partyHealthScore) do
...
-- 战斗前的苦修逻辑：血量低于PreCombatHealScore的玩家
local function PenancePreCombatLogic(partyHealthScore)
    for _, unitTable in ipairs(partyHealthScore) do
        if unitTable.healthPercent <= PreCombatHealScore then
...
-- 战斗前的耐力逻辑：给没盾的补盾
local function ShieldPreCombatLogic(partyHealthScore)
    for _, unitTable in ipairs(partyHealthScore) do
        if unitTable.shieldRemaining == 0 then
            return unitTable.unit
        end
    end
    return false
...
-- 默认盾逻辑：给非坦克玩家中，没盾，且积分最低的补盾。
local function DefaultShieldLogic(partyHealthScore)
    local party_min_health_score = 1   -- 队伍非T最低血量
    local party_min_health_score_unit = nil   -- 队伍非T最低血量的单位
    for _, unitTable in ipairs(partyHealthScore) do
        if (unitTable.role ~= "TANK") and (unitTable.shieldRemaining == 0) then
            if party_min_health_score > unitTable.healthScore then
                party_min_health_score = unitTable.healthScore
                party_min_health_score_unit = unitTable.unit
            end
        end
    end
    return party_min_health_score_unit
end
-- 苦修单体治疗逻辑：如果某个非T，血量低于60%，且玩家的治疗缺口，占整个队伍非T玩家的治疗缺口的SinglePenancePercent以上
local function SolePenanceLogic(partyHealthScore)
    local party_healing_deficit = 0   -- 队伍总治疗缺口
    for _, unitTable in ipairs(partyHealthScore) do
        if unitTable.role ~= "TANK" then
            party_healing_deficit = party_healing_deficit + unitTable.HealingDeficit
        end
    end
    for _, unitTable in ipairs(partyHealthScore) do
        if (unitTable.role ~= "TANK") and (unitTable.healthPercent < 0.6) then
            if unitTable.HealingDeficit > (party_healing_deficit * SinglePenancePercent) then
                return unitTable.unit
            end
        end
    end
...
-- 快疗逻辑：计算当前的快疗治疗量，如果有存在治疗缺口大于这个数据的玩家，则快疗
local function FlashHealLogic(partyHealthScore)
    local _, intellect = UnitStat("player", 4)-- 获取智力值
    local versatilityBonus = GetCombatRatingBonus(29) -- 全能
    local masteryEffect, bonusCoefficient = GetMasteryEffect() --精通
    local flashHealValue = intellect * (versatilityBonus / 100 + 1) * (masteryEffect / 100 + 1) * 8.25
    flashHealValue = flashHealValue * FlashHealScale

    local partyHealthScoreTemp = copyTable(partyHealthScore)
    -- 按治疗缺口排序
    table.sort(partyHealthScoreTemp, function(a, b)
        return a.HealingDeficit > b.HealingDeficit
    end)
    for _, unitTable in ipairs(partyHealthScoreTemp) do
        if unitTable.HealingDeficit > flashHealValue then
            return unitTable.unit
        end
    end
    return nil
end
-- 坦克恢复逻辑，如果坦克没有恢复，则返回坦克的名字
local function TankRenewsLogic(partyHealthScore)
    for _, unitTable in ipairs(partyHealthScore) do
        if (unitTable.role == "TANK") and (unitTable.renewsRemaining < 1) then
            return unitTable.unit
        end
    end
end
...
local function EvangelismLogic(partyHealthScore)
    ...
    local _, intellect = UnitStat("player", 4)-- 获取智力值
    local versatilityBonus = GetCombatRatingBonus(29) -- 全能
    local evangelismValue = intellect * versatilityBonus * 54   -- 福音总治疗量
    ...
    if (#partyHealthScore == 5) and (NotOverUnit >= 3) then
        return true
    end
...
-- 救赎逻辑：当ShionVars["AtonementAll"]时，返回救赎时间小于3秒的。否则，返回不满血，且没有救赎的。
local function defaultAtonementLogic(partyHealthScore)
    local NonAtonementList = {}  --无救赎角色列表

    for _, unitTable in ipairs(partyHealthScore) do
        if unitTable.atonementRemaining <= 1 then
            table.insert(NonAtonementList, unitTable)
        end
    end
    -- 并且按救赎剩余时间从小到大排序
    ...
    if #NonAtonementList > 0 then
        return NonAtonementList[1]
    end
    return nil
end
...
local function main_rotation()
    ...
    local playerTable = getPlayerTable(partyHealthScore) -- 摘取出当前玩家的状态
    ...
    local defaultTargetTable = playerTable
    ...
    if (not someOneInCombat) then
        ...
        if ShionVars["ActionInNoCombat"] then
            ...
            if PenanceCooldown then
                defaultTarget = PenancePreCombatLogic(partyHealthScore)
            ...
            if ShieldCooldown then
                defaultTarget = ShieldPreCombatLogic(partyHealthScore)
                if defaultTarget then
    ...
    ---- 保命技能处理 --
    if playerTable.healthPercent < SelfHealScore then
    ...
    if FadeCooldown then
        if playerTable.healthPercent < FadeHealScore then
    ...
    -- AOE自我套盾逻辑
    if aoeComing or inAoe then
        if playerTable.shieldRemaining == 0 then
            return Cast("player盾")
        end
    end
    ...
    if aoeComing or inAoe then
        ...
        if #nonAtonementList >= 1 then
            defaultTargetTable = nonAtonementList[1]
            -- 如果盾冷却，用盾补救赎
            if ShionVars["DEBUG"] then
                print("DEBUG: 补救赎")
            end
            if ShieldCooldown then
                return Cast(defaultTargetTable.unit .. "盾")
            end
            -- 都不行，用恢复
            return Cast(defaultTargetTable.unit .. "恢复")
        end
    ...
    if ShieldCooldown and (wealAndWoeStack >= 4) and (not HasPremonitionOfInsightBuff()) then
        --默认盾逻辑：给非坦克玩家中，没盾，且积分最低的补盾。
    ...
    if PenanceCooldown then
        -- 苦修单体治疗逻辑：如果某个非T，血量低于60%，且玩家的治疗缺口，占整个队伍非T玩家的治疗缺口的SinglePenancePercent以上
        defaultTarget = SolePenanceLogic(partyHealthScore)
        if defaultTarget then
            return Cast(defaultTarget .. "苦修")
        end
        if enemyTarget then
    ...
    if surgeOfLightCharges > 0 then
        -- 快疗逻辑：计算当前的快疗治疗量，如果有存在治疗缺口大于这个数据的玩家，则快疗
        defaultTarget = FlashHealLogic(partyHealthScore)
        if defaultTarget then
    ...
    -- 使用恢复保持坦克的救赎
    defaultTarget = TankRenewsLogic(partyHealthScore)
    if defaultTarget then
    ...
    -- 补救赎逻辑，如果有治疗缺口，用恢复
    defaultTargetTable = defaultAtonementLogic(partyHealthScore)
    if defaultTargetTable then
        return Cast(defaultTargetTable.unit .. "恢复")
    end

    if (UnitPainRemaining(enemyTarget) == 0) and enemyTarget then
    ...
    if PowerInfusionCooldown then
        defaultTarget = PowerInfusionLogic(partyHealthScore)
        return Cast(defaultTarget .. "灌注")
    end

    if CoolDown(34433) and enemyTarget then
        return Cast(enemyTarget .. "暗影魔")
    end

    -- 如果[心灵震爆]可用，则对目标释放。
    if CoolDown(8092) and (not isMoving) and enemyTarget then
