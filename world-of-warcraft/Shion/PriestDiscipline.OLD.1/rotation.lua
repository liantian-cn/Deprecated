...
-- 健康分数坦克偏移量
local Health_Score_Offset_Tank = 0.05
-- 健康分数自疗坦克偏额外移量
local Health_Score_Offset_Tank_Self_Heal = 0.1
-- 健康分数治疗偏移量
local Health_Score_Offset_Healer = -0.02
-- 健康分数脆弱职业偏移量
local Health_Score_Offset_Weak = -0.01
-- 健康分数强势职业偏移量
local Health_Score_Offset_Strong = 0.05
...
local function calculateUnitHealthScore(unitName)
    ...
    local totalAbsorbs = UnitGetTotalAbsorbs(unitName) --吸收盾
    local totalHealAbsorbs = UnitGetTotalHealAbsorbs(unitName) -- 治疗吸收盾
    local healthScore = (currentHealth + totalAbsorbs - totalHealAbsorbs) / maxHealth

    -- 根据职责微调分数
    local role = UnitGroupRolesAssigned(unitName)
    local className, classFilename, classId = UnitClass(unitName)
    if role == "TANK" then
        healthScore = healthScore + Health_Score_Offset_Tank
        if (classFilename == "DEATHKNIGHT") or (classFilename == "DEMONHUNTER") then
            healthScore = healthScore + Health_Score_Offset_Tank_Self_Heal
        end
    elseif role == "HEALER" then
        healthScore = healthScore + Health_Score_Offset_Healer
    elseif (classFilename == "HUNTER") or (classFilename == "DRUID") then
        healthScore = healthScore + Health_Score_Offset_Weak
    elseif (classFilename == "DEATHKNIGHT") or (classFilename == "DEMONHUNTER") then
        healthScore = healthScore + Health_Score_Offset_Strong
    end
    ...
    for i = 1, 40 do
        ...
        if HighDamageDebuffList[debuffData.spellId] then
            healthScore = healthScore + HighDamageDebuffHealthScoreOffset
        elseif MidDamageDebuffList[debuffData.spellId] then
            healthScore = healthScore + MidDamageDebuffHealthScoreOffset
        end
    end
...
local function DefaultShieldLogic(partyHealthScore)
    ...
    for _, item in ipairs(partyHealthScore) do
        if UnitShieldRemaining(item.name) == 0 then
            role = UnitGroupRolesAssigned(item.name)
            if role ~= "TANK" then
                return item.name
            else
                tankName = item.name
...
local function DefaultRadianceLogic(partyHealthScore)
    ...
    local count = 0 -- 血量低于90%,且有救赎的玩家的数量
    for _, item in ipairs(partyHealthScore) do
        if (item.value < 0.90) and (UnitAtonementRemaining(item.name) < 1) then
            count = count + 1
            if count >= 2 then
...
local function main_rotation()
    ...
    -- 自动套盾
    local ShieldCooldown = CoolDown(17)
    ...
    -- 苦修卡CD用
    local PenanceCooldown = CoolDown(47540, 700) --苦修CD
    ...
    if surgeOfLightCharges > 0 then
        defaultTarget = DefaultFlashHealLogic(partyHealthScore)
        if defaultTarget then
